"""Quality gates for concise Venture Artifacts and Artifact-derived Memory."""
...
_ALLOWED_CATEGORIES = {"finding", "decision", "risk", "open_question", "constraint", "next_step", "contradiction"}
...
def is_semantic_title(value: Any) -> bool:
    ...
    if _normal(title) in _GENERIC or not re.search(r"[A-Za-z]", title):
        return False
    if re.fullmatch(r"\s*\d+[.)-]?\s*", title):
        return False
    if re.match(r"^\s*(?:[CSA]\d+|\d+)\s*[.)-]", title, re.I):
        return False
    if re.search(r"\[(?:C|S|A)\d+\]", title):
        return False
    if re.match(r"^(?:matthew|mark|luke|john|acts|romans|genesis|exodus)\s+\d+[:.]\d+", title, re.I):
        return False
...
def _evidence_echo(text: str, citations: list[str], labels: dict[str, dict[str, Any]]) -> bool:
    statement = _clean(text, 800)
    if len(statement) > 360 or _numbered_passage(statement):
        return True
...
def is_revelation(value: Any, citations: list[str] | None = None, labels: dict[str, dict[str, Any]] | None = None) -> bool:
    ...
    if len(text) < 24 or len(_words(text)) < 6 or _numbered_passage(text):
        return False
    return labels is None or not _evidence_echo(text, citations or [], labels)


def _category(title: str, revelation: str, requested: Any) -> str:
    category = _clean(requested).lower()
    if category in _ALLOWED_CATEGORIES:
        return category
    text = f"{title} {revelation}".lower()
    if "?" in revelation or "question" in text:
        return "open_question"
    if "risk" in text:
        return "risk"
    if "constraint" in text or "must" in text:
        return "constraint"
    if "decision" in text or "decided" in text:
        return "decision"
    if "should" in text or "next step" in text:
        return "next_step"
    return "finding"
...
def validate_synthesis_json(data: dict[str, Any], labels: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    observations = []
    for item in (data.get("conversation_observations") or data.get("observations") or [])[:3]:
        if not isinstance(item, dict):
            continue
        citations = _citations(item.get("citations"), valid)
        conversation = [citation for citation in citations if citation.startswith("C")]
        if not conversation:
        ...
        if not text:
            continue
        kind = _clean(item.get("kind") or "observation").lower()
        if kind not in {"request", "observation", "decision", "constraint", "question", "inference", "risk"}:
            kind = "observation"
        speaker = _clean(item.get("speaker"), 120)
        if not speaker:
            speaker = next((_clean(labels[c].get("speaker"), 120) for c in citations if labels[c].get("speaker")), "Participant")
        observations.append({"speaker": speaker, "kind": kind, "text": text, "citations": citations})
    if not observations:
        return {"should_create": False, "reason": "insufficient_grounded_conversation"}

    questions = []
    for item in (data.get("questions_raised") or [])[:6]:
        if not isinstance(item, dict):
            continue
        citations = _citations(item.get("citations"), valid)
    ...
    points = []
    seen = set()
    for item in (data.get("key_points") or [])[:5]:
        if not isinstance(item, dict):
            continue
        citations = _citations(item.get("citations"), valid)
        title = _clean(item.get("title"), 120)
        revelation = _clean(item.get("revelation") or item.get("memory") or item.get("content"), 360)
        ...
        if signature in seen:
            continue
        seen.add(signature)
        confidence = _clean(item.get("confidence") or "medium").lower()
        points.append({
            "title": title,
            "revelation": revelation,
            "content": revelation,
            "category": _category(title, revelation, item.get("category")),
    ...
    return {
        ...
        "recommended_next_bearing": [_clean(step, 240) for step in (data.get("recommended_next_bearing") or [])[:6] if _clean(step)],
    }
...
def render_artifact_markdown(synthesis: dict[str, Any], labels: dict[str, dict[str, Any]]) -> str:
    ...
    times = [labels[label].get("captured_at") for label in cited if labels[label].get("captured_at")]
    window = f"{min(times)}–{max(times)}" if times else "unspecified"
    lines = [
        f"# {synthesis['title']}", "",
    ...
    for number, observation in enumerate(synthesis.get("conversation_observations") or [], 1):
        citations = " ".join(f"[{citation}]" for citation in observation.get("citations") or [] if citation in labels)
        lines.extend(["", f"{number}. **{observation['speaker']} — {observation['kind'].title()}**: {observation['text']} {citations}".rstrip()])
    if synthesis.get("questions_raised"):
        lines.extend(["", "## Questions raised"])
        for question in synthesis["questions_raised"]:
            citations = " ".join(f"[{citation}]" for citation in question.get("citations") or [] if citation in labels)
            lines.append(f"- {question['text']} {citations}".rstrip())
    lines.extend(["", "## Key points"])
    for number, point in enumerate(synthesis.get("key_points") or [], 1):
        citations = " ".join(f"[{citation}]" for citation in point.get("citations") or [] if citation in labels)
        lines.extend(["", f"{number}. **{point['title']}**", f"**Revelation:** {point['revelation']} {citations}".rstrip()])
    ...
    for label in cited:
        item = labels[label]
        supports = [point["revelation"][:100] for point in synthesis.get("key_points") or [] if label in point.get("citations") or []]
...
def _evidence_index(content: str) -> dict[str, dict[str, str]]:
    ...
    for raw in content.splitlines():
        ...
        if active and line.startswith("## "):
            break
        if not active or not line.startswith("|") or line.startswith("|---") or " ID " in line:
            continue
...
def extract_artifact_key_point_candidates(proposal) -> list[dict[str, Any]]:
    ...
    for raw in content.splitlines():
        ...
        if heading:
            ...
            continue
        revelation = re.match(r"^\*\*Revelation:\*\*\s*(.+)$", line, re.I)
...
A semantic Artifact title has 3–12 words and never uses a list number, source locator, citation, or generic label.
Create 1–3 chronological conversation observations, cited questions, and 3–5 key points.
Every key point requires title, revelation, category, confidence, and citations.
A revelation is a short, original explanation of the implication, decision, relationship, risk, or next move the crew should retain.
Never copy source passages, verse text, transcripts, numbered list items, citations, or quotations into a revelation.
Memories are extracted from revelations, so each must stand alone without reopening the Artifact.
"""
    ...
    def process_artifact_memory_synthesis(db, job):
        ...
        if proposal:
            for row in db.query(synthesis.QuestMemoryEntry).filter(
