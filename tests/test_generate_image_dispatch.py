...
def test_json_agent_payload_is_normalized_before_local_generation(monkeypatch):
    image_calls = []
    ...
    async def generate(content, session_id=None, owner=None):
        image_calls.append(content)
        return {"results": "Generated image", "image_url": "/api/generated-image/dragon.png"}

    monkeypatch.setattr(ai_interaction, "do_generate_image", generate)
    block = SimpleNamespace(
        tool_type="generate_image",
        content=(
            '{"prompt":"A majestic dragon at sunset","model":"gpt-image-1",'
    ...
    assert desc == "generate_image: A majestic dragon at sunset"
    assert result["exit_code"] == 0
    assert image_calls == ["A majestic dragon at sunset\ngpt-image-1\n1024x1024\nhigh"]
