...
import { renderVideoJobCard } from './mediaJobRenderer.js';
...
const _assistantName = () => window.ARGOS_BRAND?.assistant || 'Odysseus';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = _assistantName();
      ...
      for (let r = 0; r < maxRound; r++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            ...
            if (ev.kind === 'video_generation' || ev.video_job_id || ev.job_id) {
              renderVideoJobCard(node.querySelector('.agent-thread-content'), ev);
            }
          }
